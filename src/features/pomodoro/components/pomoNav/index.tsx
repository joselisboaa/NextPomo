...
export const PomodoroNav = () => {
  const { definePomodoroStatus, standardBreak, shortBreak } = useContext(CountDownContext);

  return (
    <div className={twMerge("flex h-3.5 w-fit items-center justify-center rounded-full bg-primary md:h-[3.5rem]")}>
      <button
        onClick={() => definePomodoroStatus(false, true, standardBreak)}
      ...
      <button
        onClick={() => definePomodoroStatus(true, true, shortBreak)}
      ...
      <button
        onClick={() => definePomodoroStatus(true, true, standardBreak)}
